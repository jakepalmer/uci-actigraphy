...
fit <- lapply(file_list, function (file) {
    ...
    df_subj$timehour_shifted <- ifelse(df_subj$timehour < 12.00, df_subj$timehour + 24, df_subj$timehour)
    ...
    pred_data$time <- pred_data$time - 12.00
    pred_data$time_hr <- floor(pred_data$time)
    pred_data$time_min <- sprintf("%02d", round((pred_data$time - pred_data$time_hr)*60, 0))
    pred_data <- transform(pred_data, time_24hr=paste(time_hr, time_min, sep=":"))
    pred_data$time_24hr <- strptime(pred_data$time_24hr, format = "%H:%M")
    pred_data$time_24hr <- strftime(pred_data$time_24hr, format = "%H:%M")
    print(tail(pred_data))
    #---
    p <- ggplot(pred_data, aes(x = time_24hr, group = 1)) + 
